...
#### Manual setup
Download dataset from [here](https://drive.google.com/file/d/10Aqv57jU1RPsf2duOPHJe2asc6HYHOGc/view?usp=sharing) and save the dataset folder in the `data` subfolder.

Download trained weights from [here](https://dl.dropboxusercontent.com/s/vlz8ip8hh4nwmpa/gen_dis_default.zip?dl=0) and save the `.pth` file in the `weights` subfolder.

## Data
The dataset consists of 16 images of class IV neurons at a resolution of 512x512 and can be downloaded from [here](https://drive.google.com/file/d/10Aqv57jU1RPsf2duOPHJe2asc6HYHOGc/view?usp=sharing).
...
* Duplicate the `config_default.py` file in the `configs` folder and rename it to `my_config.py`.
* Define the `ID` variable in `my_config.py` indicating the training run ID.
...
## Evaluation
* Download the discriminator and generator weights from [here]() and save the `.pth` file in the `weights` subfolder.
...
$$ \mathcal{L}_{D} = - \langle D(x)^2 \rangle_{x \in Real} + \langle D(G(z)) \rangle_{z \in L} + \lambda \langle (||\nabla D(y)||_2 - 1)^2 \rangle_{y} + \epsilon_{\text{drift}} \langle D(x)^2 \rangle_{x \in Real}$$
...
The third term corresponds to the gradient penalty term introduced in [Improved Training of Wasserstein GANs, 2017](http://arxiv.org/abs/1704.00028). This term is added as an alternative to weight clipping. The gradient is taken over random interpolations betwee real and generated images: $y = t x + (1-t)G(z)$ where $t$ is random and uniformly-distributed in $[0,1]$. $\lambda=10$ is used to weight the penalty term.
...
The samples are created using fixed randomly sampled latent vectors. The resolution increases by a factor of 2 every 50k epochs.

https://user-images.githubusercontent.com/35376874/222034762-a13ac5b2-1c82-4bf3-a698-2c09eed2e17e.mp4
