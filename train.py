...
from loss_functions import D_W_loss, G_W_loss, similarity_loss, D_grad_loss, D_drift_loss
...
parser.add_argument('--grad_lambda', type=float, default=0.0,
...
# Overwrite the configurations given in the configs file
if options.configs:
    config.import_configs(options.configs)

# Overwrite configs with the input arguments
...
from data.NeuronDataset import NeuronDataset
from models import *
...
samples_sub_dir = config.samples_sub_dir
if os.path.exists(samples_sub_dir) and not config.resume:
...
if device.type == 'mps':
    # MPS only supports 32 bit floats for now.
...
if config.device == 'cuda' and config.pin_memory:
    config.N_workers = 0
...
# Find the inital and final image size for PGGAN
if config.pggan:
...
checkpoint = Checkpointer(Generator_net, Discriminator_net, config.learning_rate, Train_state_filename, device=device)
...
lr_decay_rate = []
transitions_epoch = [0] + config.transit_sch
for i in range(len(transitions_epoch) - 1):
    Phase_N_Epochs = transitions_epoch[i + 1] - transitions_epoch[i]
...
def update_lr(optimizer, epoch):
    ...
    if epoch in transitions_epoch:
        ...
        Phase_N_Epochs = transitions_epoch[Phase_ind + 1] - transitions_epoch[Phase_ind]
        epoch_since_last_transit = epoch - transitions_epoch[Phase_ind]
...
Loss_G_series = checkpoint.Loss_G
Loss_D_series = checkpoint.Loss_D
...
Monitoring_period = 10 if config.device == 'cuda' else 1
...
def pggan_train():
    ...
    for epoch in range(epoch_init, config.N_epochs + epoch_init):
        ...
        for (i, images) in enumerate(dataloader):
            ...
            for j in range(N_D_steps):
                ...
                D_loss, Score_real, Score_fake = D_W_loss(Generator_net, Discriminator_net, images)

                # Add gradient penalty
                D_grad_pen = D_grad_loss(Generator_net, Discriminator_net, images, Lambda=config.grad_lambda)
                D_loss += D_grad_pen

                # Add drift loss
                D_dr_loss = D_drift_loss(Discriminator_net, images, epsilon=config.drift_epsilon)
                D_loss += D_dr_loss
            ...
            G_loss, Z_latent = G_W_loss(Generator_net, Discriminator_net, images)
            ...
            epoch_score_real += Score_real.detach().item()
            epoch_score_fake += Score_fake.detach().item()
            epoch_D_loss += D_loss.detach().item()  # Only accumulate the loss of the last discriminator iteration
            epoch_G_loss += G_loss.detach().item()
            epoch_D_grad_pen += D_grad_pen.detach().item()
            epoch_G_sim_loss += G_sim_loss.detach().item()
        ...
        if epoch % Monitoring_period == 0:
            ...
            Monitored_values['Loss_real (<D(x)>_x)'] = '{: >#7.4g}'.format(epoch_score_real)
            Monitored_values['Loss_fake (<D(G(z))>)'] = '{: >#7.4g}'.format(epoch_score_fake)
            Monitored_values['G_loss'] = '{: >#7.4g}'.format(epoch_G_loss)
            Monitored_values['D_loss'] = '{: >#7.4g}'.format(epoch_D_loss)
            if epoch_D_grad_pen != 0:
                Monitored_values['D_grad_pen'] = '{: >#7.4g}'.format(epoch_D_grad_pen)
            if epoch_G_sim_loss != 0:
                Monitored_values['G_sim_loss'] = '{: >#7.4g}'.format(epoch_G_sim_loss)
        ...
        Score_real_series.append(epoch_score_real)
        Score_fake_series.append(epoch_score_fake)
        Loss_G_series.append(epoch_G_loss)
        Loss_D_series.append(epoch_D_loss)

        # Checkpoint
        if epoch % config.checkpointing_period == 0:
            # Save weights
            checkpoint.save_state(epoch, Score_real_series, Score_fake_series, Loss_G_series, Loss_D_series)
            ...
            Fake_samples_filepath = os.path.join(samples_sub_dir, 'Samples_{}_{:d}.png'.format(config.ID, epoch))
...
def wgan_train():
    ...
    for epoch in range(epoch_init, config.N_epochs + epoch_init):
        ...
        for (i, images) in enumerate(dataloader):
            ...
            epoch_D_loss += D_loss.item()  # Only accumulate the loss of the last discriminator iteration
...
def wgan_unroll_train(N_unroll_steps):
    LATENT_DIM = 100  # Dimension of the latent space of the generator

    Dis_loss_real = None
    Dis_loss_fake = None
    dis_output_real = None
    dis_output_fake = None
    Discriminator_net_state_dict = None
    for epoch in range(1, config.N_epochs + 1):
        ...
        for (i, images) in enumerate(dataloader, start=0):
            ...
            # Train generator and discriminator for the number of unroll steps.
            for j in range(N_unroll_steps):
                # Zero gradients and setup labels
                Discriminator_net.zero_grad()
                ...
                # Calculate discriminator loss on real images (- <D(x)>_x)
                dis_output_real = Discriminator_net(images)
                Dis_loss_real = - dis_output_real.mean()

                # Calculate discriminator loss on fake images (<D(G(z))>_z)
                # detach fake_images to avoid computing grads on generator
                dis_output_fake = Discriminator_net(fake_images.detach())
                Dis_loss_fake = dis_output_fake.mean()

                # Backprop and gradient descent on discriminator
                ...
                # Save the discriminator weights at the first iteration for unrolling
                if config.unroll_steps > 0 and j == 0:
                    Discriminator_net_state_dict = Discriminator_net.state_dict().copy()

            # Unroll the gradient descent steps of the discriminator
            if config.unroll_steps > 0:
                Discriminator_net.load_state_dict(Discriminator_net_state_dict)
        ...
        Score_real_series.append(epoch_D_loss_real)
        Score_fake_series.append(epoch_D_loss_fake)
        Loss_G_series.append(epoch_G_loss)
        ...
        if epoch % config.checkpointing_period == 0:
            ...
            # Update the training summary plot
            plot_scores(Score_real_series, Score_fake_series, Loss_G_series, training_summary_filename)
...
if config.unroll_steps > 0:
    wgan_unroll_train(config.unroll_steps)
